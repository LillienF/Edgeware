...
def resize(img:Image.Image, screen_width, screen_height) -> Image.Image:
...
def run():
    ...
    resized_image = resize(image, SCREEN_WIDTH, SCREEN_HEIGHT)
    ...
    if video_mode:
        #video mode
        label = VideoLabel(bg, name='!imgLabel')
        ...
        #gif mode
        label = GifLabel(bg, name='!imgLabel')
        ...
        if not SUBLIMINAL_MODE:
            label = Label(bg, image=photoimage_image, bg='black', name='!imgLabel')
            label.pack()
        else:
            label = GifLabel(bg, name='!subliminalLabel')
        ...
        if do_deny:
            ...
            denyLabel = Label(label, text=deny_text, name='!denyLabel')
    ...
    if LOWKEY_MODE:
        thread.Thread(target=lambda: lowkey_loop(root, DELAY / 1000), daemon=True).start()

    if SHOW_CAPTIONS and CAPTIONS:
        caption_text = select_caption(item)
        if caption_text is not None:
            captionLabel = Label(root, text=caption_text, wraplength=resized_image.width - border_wid_const, name='!captionLabel')
...
def lowkey_loop(parent:tk, length:int):
    while True:
        time.sleep(length)
        img = GetRandomImage()
        print(img.filename)
        resized_image = resize(img, SCREEN_WIDTH, SCREEN_HEIGHT)
        photoimage_image = ImageTk.PhotoImage(resized_image)
        img.close()
        gif_bool = img.filename.split('.')[-1].lower() == 'gif'
        canvas = parent.nametowidget('.!canvas')
        try:
            parent.nametowidget('.!canvas.!imgLabel').destroy()
        except Exception as e:
            print(str(e))
            raise e
        ...
        if gif_bool:
            #gif mode
            label = GifLabel(canvas, name='!imgLabel')
            label.load(path=os.path.abspath(img.filename), resized_width = resized_image.width, resized_height = resized_image.height)
            label.pack()
        else:
            #standard image mode
            if not SUBLIMINAL_MODE:
                label = Label(canvas, image=photoimage_image, bg='black', name='!imgLabel')
                label.pack()
        ...
        parent.geometry(f'{resized_image.width + 5 - 1}x{resized_image.height + 5 - 1}+{locX}+{locY}')

        if gif_bool:
            label.next_frame()

        # This might work??
        # thread.Thread(target=lambda: lowkey_loop(parent, DELAY / 1000), daemon=True).start()

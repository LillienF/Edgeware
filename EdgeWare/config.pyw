...
def show_window():
    ...
    while(fail_loop < 2):
        try:
            ...
            # ctToggle            = BooleanVar(root, value=int(settings['ctToggle']) if settings['ctToggle'] else 1)
    ...
    # clickThroughToggle = Checkbutton(lowkeyFrame, text='Click-Trough', variable=ctToggle, command=lambda: toggleAssociateSettings(ctToggle.get(), lowkey_group))
    ...
    # clickThroughToggle.pack(fill='both', expand=1)
...
def write_save(varList:list[StringVar | IntVar | BooleanVar], nameList:list[str], passVar:str, exitAtEnd:bool):
    ...
    if int(varList[nameList.index('timerMode')].get()) == 1:
        ...
        with open(hashObjPath, 'w') as passFile, open(timeObjPath, 'w') as timeFile:
            ...
            timeFile.write(str(varList[nameList.index('timerSetupTime')].get()*10))
